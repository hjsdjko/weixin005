	...
	<view :style='{"width":"100%","padding":"0 0 0 0","position":"relative","background":"#ff993310","height":"auto"}'>
		<form :style='{"width":"100%","padding":"24rpx 24rpx 24rpx 24rpx","background":"none","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"4rpx 0 4rpx 0","margin":"0 0 24rpx 0","borderColor":"#eeeeee","alignItems":"center","borderWidth":"0px","background":"none","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				...
				<textarea :style='{"border":"2rpx solid #eeeeee","padding":"24rpx 24rpx 24rpx 24rpx","margin":"0 0 0 0","color":"#000000","borderRadius":"0px","flex":"1","background":"#ffffff","fontSize":"28rpx","height":"240rpx"}' v-model="ruleForm.content" placeholder="评论内容"></textarea>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussmenzhenyisheng`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussmenzhenyisheng`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussmenzhenyisheng`, this.ruleForm);
						}else{
							await this.$api.add(`discussmenzhenyisheng`, this.ruleForm);
						}
						uni.setStorageSync('discussmenzhenyishengCleanType',true);
