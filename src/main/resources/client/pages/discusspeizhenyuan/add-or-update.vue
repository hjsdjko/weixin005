	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusspeizhenyuan`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusspeizhenyuan`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusspeizhenyuan`, this.ruleForm);
						}else{
							await this.$api.add(`discusspeizhenyuan`, this.ruleForm);
						}
						uni.setStorageSync('discusspeizhenyuanCleanType',true);
