...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusspeizhenyuanCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusspeizhenyuanCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discusspeizhenyuan', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusspeizhenyuan');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discusspeizhenyuan');
			...
			async init(){
				...
				let res = await this.$api.info('discusspeizhenyuan', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusspeizhenyuan', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscusspeizhenyuan/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
