...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view class="container" :style='{"width":"100%","padding":"0px","position":"relative","background":"#ff993310","height":"auto"}'>
				...
				<view :style='{"margin":"12rpx 0 12rpx 0","borderColor":"#cccccc","flexWrap":"wrap","borderWidth":"0 0 0px 0","background":"none","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="detail-list-item title">
					<view :style='{"width":"auto","padding":"0 20rpx 0 0","lineHeight":"48rpx","fontSize":"28rpx","color":"#333333","textAlign":"right"}' class="lable">挂号费：</view>
					<view :style='{"padding":"0px 0 0 0","margin":"0 0 0 0","lineHeight":"48rpx","fontSize":"28rpx","color":"#666666","flex":"1"}' class="text">{{detail.guahaofei}}</view>
				</view>
				...
				<view class="detail-list-item" :style='{"margin":"12rpx 0 12rpx 0","borderColor":"#cccccc","flexWrap":"wrap","borderWidth":"0 0 0px 0","background":"none","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
					...
					<view class="text" :style='{"padding":"0px 0 0 0","margin":"0 0 0 0","lineHeight":"48rpx","fontSize":"28rpx","color":"#666666","flex":"1"}'>{{detail.jianjie}}</view>
				</view>
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"padding":"0","margin":"20rpx 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx 0 20rpx","margin":"0 20rpx 12rpx 0","color":"#fff","borderRadius":"0","background":"#ff9933","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="userid&&isAuth('menzhenyisheng','挂号')" @tap="onAcrossTap('yuyueguahao','','','')" class="cu-btn bg-brown round shadow-blur" >挂号</button>
					<button :style='{"border":"0","padding":"0 20rpx 0 20rpx","margin":"0 20rpx 12rpx 0","color":"#fff","borderRadius":"0","background":"#ff9933","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="!userid&&isAuthFront('menzhenyisheng','挂号')" @tap="onAcrossTap('yuyueguahao','','','')" class="cu-btn bg-brown round shadow-blur" >挂号</button>
				</view>
			</view>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			this.getStoreup();
			let cleanType = uni.getStorageSync('discussmenzhenyishengCleanType')
			if(cleanType){
				uni.removeStorageSync('discussmenzhenyishengCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('menzhenyisheng', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','menzhenyisheng');
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								...
								await _this.$api.update('menzhenyisheng',_this.detail)
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.add('storeup', {
								userid: _this.user.id,
								name: _this.detail.yishengzhanghao,
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','menzhenyisheng');
			...
			async init(){
				...
				let res = await this.$api.info('menzhenyisheng', this.id);
				this.detail = res.data;
				// 轮播图片
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussmenzhenyisheng', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussmenzhenyisheng/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
